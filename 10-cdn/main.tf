resource "aws_cloudfront_distribution" "jaya123" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.jaya123.domain_name
        zone_id = aws_cloudfront_distribution.jaya123.hosted_zone_id
      }
    }
  ]
}